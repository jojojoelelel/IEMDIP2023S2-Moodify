...
(function () {
  ...
  const parseVecRange = (str, def) => {
    ...
    return flattenDeep(
      parts.map(a =>
        def.map((x, i) => (typeof a[i] === 'undefined' ? x : a[i])),
      ),
    );
  };
  ...
  let WHITE_TEXTURE = new THREE.DataTexture(
  ...
  AFRAME.registerComponent('sprite-particles', {
    schema: {
      ...
      trailType: {
        default: 'particle',
        oneOf: ['particle', 'ribbon', 'ribbon3d'],
      },
      ribbonWidth: {default: 1},
      ribbonShape: {
        default: 'flat',
        oneOf: ['flat', 'taperin', 'taperout', 'taper'],
        parse: toLowerCase,
      },
      ribbonUVType: {
        default: 'overtime',
        oneOf: UV_TYPE_STRINGS,
        parse: toLowerCase,
      },
      ...
      radialType: {
        default: 'circle',
        oneOf: ['circle', 'sphere', 'circlexy', 'circlexz'],
        parse: toLowerCase,
      },
    ...
    update(oldData) {
      ...
      if (data.overTimeSlots !== oldData.overTimeSlots && !this.isPlaying) {
        ...
        this.colorOverTime = new Float32Array(
          4 * this.overTimeArrayLength,
        ).fill(0); // color is xyz and opacity is w
        this.rotationScaleOverTime = new Float32Array(
          2 * this.overTimeArrayLength,
        ).fill(0); // x is rotation, y is scale
        overTimeDirty = true;
      }
      ...
      this.textureFrames[2] =
        data.textureCount > 0
          ? data.textureCount
          : data.textureFrame.x * data.textureFrame.y;
      ...
      if (data.ribbonUVType !== oldData.ribbonUVType) {
        this.params[RIBBON_UV_TYPE_PARAM] =
          UV_TYPE_STRINGS.indexOf(data.ribbonUVType) === -1
            ? 0
            : UV_TYPE_STRINGS.indexOf(data.ribbonUVType);
      }

      if (data.radialType !== oldData.radialType) {
        this.params[RADIAL_X_PARAM] = ['sphere', 'circlexy', 'circle'].includes(
          data.radialType,
        )
          ? 1
          : 0;
        this.params[RADIAL_Y_PARAM] = ['sphere', 'circlexz'].includes(
          data.radialType,
        )
          ? 1
          : 0;
      ...
      if (
        ...
        data.radialPosition !== oldData.radialPosition
      ) {
      ...
      if (
        data.velocity !== oldData.velocity ||
        data.radialVelocity !== oldData.radialVelocity
      ) {
      ...
      if (
        data.acceleration !== oldData.acceleration ||
        data.radialAcceleration !== oldData.radialAcceleration
      ) {
      ...
      if (
        data.rotation !== oldData.rotation ||
        data.scale !== oldData.scale ||
        overTimeDirty
      ) {
      ...
      if (data.lifeTime !== oldData.lifeTime) {
        this.lifeTime = this.updateVec4WRange(
          data.lifeTime,
          [1],
          'angularVelocity',
        );
      ...
      if (data.trailLifeTime !== oldData.trailLifeTime) {
        ...
        this.trailLifeTime = parseVecRange(data.trailLifeTime, [0]).map(
          (x, i) => (x > 0 ? x : this.lifeTime[i]),
        );
      ...
      if (data.angularAcceleration !== oldData.angularAcceleration) {
        this.updateAngularVec4XYZRange(
      ...
      if (data.orbitalVelocity !== oldData.orbitalVelocity) {
        this.updateAngularVec2PartRange(
      ...
      if (data.destinationOffset !== oldData.destinationOffset) {
        this.destinationOffset = this.updateVec4XYZRange(
      ...
      if (data.destinationWeight !== oldData.destinationWeight) {
        this.destinationWeight = this.updateVec4WRange(
          data.destinationWeight,
          [0],
          'destination',
        );
      }
      ...
      if (
        data.duration !== oldData.duration ||
        data.delay !== oldData.delay ||
        data.emitterTime !== oldData.emitterTime
      ) {
      ...
      if (
        data.spawnType !== oldData.spawnType ||
        data.spawnRate !== oldData.spawnRate ||
        data.lifeTime !== oldData.lifeTime ||
        data.trailInterval !== oldData.trailInterval
      ) {
        const maxParticleLifeTime = this.lifeTime[1];
        const maxTrailLifeTime =
          data.trailInterval > 0 ? this.trailLifeTime[1] : 0;
        const maxAge = maxParticleLifeTime + maxTrailLifeTime;
        const particleCount = Math.max(1, Math.ceil(maxAge * data.spawnRate));
        this.trailCount =
          1 +
          (data.trailInterval > 0
            ? Math.ceil(
                Math.min(maxTrailLifeTime, maxParticleLifeTime) /
                  data.trailInterval,
              )
            : 0); // +1 because the trail includes the lead particle
      ...
      if (
        ...
        data.model &&
        'getObject3D' in data.model
      ) {
        if (oldData.model) {
          oldData.model.removeEventListener(
            'object3dset',
            this.handleObject3DSet,
          );
        }
        this.updateModelMesh(data.model.getObject3D(MODEL_MESH));
        if (data.model) {
          data.model.addEventListener('object3dset', this.handleObject3DSet);
        }
      }

      if (data.particleOrder !== 'original' && data.source) {
        console.warn(
          `changing particleOrder to 'original' (was '${data.particleOrder}'), because particles use a source`,
        );
      }
      ...
      this.manageIDs =
        ...
        !data.enable ||
        data.source ||
        typeof this.el.getDOMAttribute(this.attrName).enable !== 'undefined' ||
        data.model ||
        data.delay > 0;
    ...
    tick(time, deltaTime) {
      ...
      if (!data.model || this.modelVertices) {
        ...
        if (
          data.destination &&
          data.destination.object3D &&
          (this.destinationWeight[0] > 0 || this.destinationWeight[1] > 0)
        ) {
    ...
    handleObject3DSet(event) {
      if (
        event.target === this.data.model &&
        event.detail.type === MODEL_MESH
      ) {
    ...
    updateBounds() {
      ...
      for (let j = 0; j < 2; j++) {
        ...
        for (let i = 0; i < STRIDE; i++) {
          // 0 = x, 1 = y, 2 = z, 3 = radial
          const offset = compare(this.offset[i], this.offset[i + STRIDE]);
          const velocity = compare(this.velocity[i], this.velocity[i + STRIDE]);
          const acceleration = compare(
            this.acceleration[i],
            this.acceleration[i + STRIDE],
          );

          // extent at time tmax
          extent[j][i] =
            offset + (velocity + 0.5 * acceleration * maxAge) * maxAge;
          ...
          if (turningPoint > 0 && turningPoint < maxAge) {
            extent[j][i] = compare(
              extent[j][i],
              offset - (0.5 * velocity * velocity) / acceleration,
            );
          }
        }
      }
      ...
      const maxScale = this.rotationScaleOverTime.reduce(
        (max, x, i) => (i & 1 ? Math.max(max, x) : max),
        0,
      ); // scale is every second number
      const maxRadial =
        Math.max(Math.abs(extent[0][3]), Math.abs(extent[1][3])) +
        data.particleSize * 0.00045 * maxScale;
      const isSphere =
        data.radialType === 'sphere' ||
        domAttrs.angularVelocity ||
        domAttrs.angularAcceleration ||
        domAttrs.orbitalVelocity ||
        domAttrs.orbitalAcceleration;
      ...
      const maxR = Math.max(
        ...extent[0].map(Math.abs),
        ...extent[1].map(Math.abs),
      );
    ...
    updateDestinationEntity: (function () {
      ...
      return function updateDestinationEntity() {
        ...
        for (let i = 0, n = AXES_NAMES.length; i < n; i++) {
          ...
          this.destination[i + 4] =
            dest[AXES_NAMES[i]] + this.destinationOffset[i + 3]; // max part of range
        }
      };
    })(),
    ...
    enableEditorObject(enable) {
      ...
      if (enable && (!existingMesh || existingMesh.isParticlesEditorObject)) {
        const BOX_SIZE = 0.25;
        const maxBound = new THREE.Vector3(BOX_SIZE, BOX_SIZE, BOX_SIZE).max(
          this.geometry.boundingBox.max,
        );
        const minBound = new THREE.Vector3(-BOX_SIZE, -BOX_SIZE, -BOX_SIZE).min(
        ...
        !enable &&
        existingMesh &&
        existingMesh.isParticlesEditorObject
      ) {
    ...
    updateAttributes() {
      if (this.geometry) {
        ...
        this.geometry.setAttribute(
          'vertexID',
          new THREE.Float32BufferAttribute(vertexIDs, 1),
        ); // gl_VertexID is not supported, so make our own id
        this.geometry.setAttribute(
          'position',
          new THREE.Float32BufferAttribute(new Float32Array(n * 3).fill(0), 3),
        );
    ...
    updateDefines() {
      ...
      if (data.source) {
        ...
        defines.PARTICLE_ORDER = PARTICLE_ORDER_STRINGS.indexOf(
          data.particleOrder,
        );
      }
      defines.PARTICLE_TRAIL_ORDER = PARTICLE_ORDER_STRINGS.indexOf(
        data.particleOrder,
      );

      const extraDefines = Object.keys(defines).filter(
        b => this.material.defines[b] !== defines[b],
      );
      ...
      if (extraDefines.length > 0) {
        if (this.isPlaying) {
          ...
          console.error(
            `cannot add attributes (${extraAttrs.join(',')}) at run-time`,
          );
        } else {
    ...
    updateModelMesh(mesh) {
      ...
      const parseModel = obj3D => {
        ...
        if (positions && positions.itemSize !== 3) {
          return;
        } // some text geometry uses 2D positions
    ...
    updateWorldTransform: (function () {
      ...
      return function (emitterTime) {
        ...
        const isEnableDisable = data.enable
          ? this.numEnabled < n
          : this.numDisabled < n;
        ...
        switch (data.modelFill) {
          case 'edge':
            modelFillFn = randomPointOnTriangleEdge;
            break;
          case 'vertex':
            modelFillFn = randomVertex;
            break;
        }
        ...
        while (this.nextTime < emitterTime && numSpawned < this.count) {
          ...
          for (
            let particleVert = 0,
              particleVertCount = isRibbon ? VERTS_PER_RIBBON : 1;
            particleVert < particleVertCount;
            particleVert++
          ) {
        ...
        if (numSpawned > 0) {
          const trailVertCount =
            this.trailCount * (isRibbon ? VERTS_PER_RIBBON : 1);
          this.params[ID_PARAM] = Math.floor(id / trailVertCount); // particle ID

          if (isBurst) {
            // if we did burst emit, then wait for maxAge before emitting again
  ...
  const randomPointInTriangle = (function () {
    ...
    return function randomPointInTriangle(vertices, pos) {
      ...
      let triangleOffset =
        Math.floor((Math.random() * vertices.length) / 9) * 9;
    ...
    return function randomPointOnTriangleEdge(vertices, pos) {
      ...
      if (r1 > 2 / 3) {
        pos
          .copy(v1)
          .sub(v3)
          .multiplyScalar(r1 * 3 - 2)
          .add(v3);
      } else if (r1 > 1 / 3) {
        pos
          .copy(v3)
          .sub(v2)
          .multiplyScalar(r1 * 3 - 1)
          .add(v2);
      } else {
        pos
          .copy(v2)
