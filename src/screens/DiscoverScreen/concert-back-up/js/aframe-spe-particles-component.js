...
})()(
  {
    1: [
      function (require, module, exports) {
        ...
        const parseFloatArray = a =>
          typeof a === 'string' ? a.split(',').map(y => parseFloat(y)) : a;
        ...
        AFRAME.registerComponent('spe-particles', {
          schema: {
            enableInEditor: {
              default: false,
              description:
                'enable the emitter while the editor is active (i.e. while scene is paused)',
            },
            enabled: {
              default: true,
            ...
            texture: {
              type: 'map',
              description:
                'texture to be used for each particle, may be a spritesheet',
            },
            textureFrames: {
              type: 'vec2',
              default: {x: 1, y: 1},
              description:
                'x and y frames for a spritesheet. each particle will transition through every frame of the spritesheet over its lifetime (see textureFramesLoop)',
            },
            textureFrameCount: {
              type: 'int',
              default: -1,
              description:
                'number of frames in the spritesheet, negative numbers default to textureFrames.x * textureFrames.y',
            },
            textureFrameLoop: {
              type: 'int',
              default: 1,
              description:
                'number of times the spritesheet should be looped over the lifetime of a particle',
            },
            // maxParticleCount: {
            //   default: 1000,
            ...
            hasPerspective: {
              default: true,
              description:
                'if true, particles will be larger the closer they are to the camera. setting this to false cancels the effect of emitterScale',
            },
            useTransparency: {
              default: true,
              description:
                'should the particles be rendered with transparency?',
            },
            alphaTest: {
              default: 0,
              min: 0,
              max: 1,
              description:
                'alpha values below the alphaTest threshold are considered invisible',
            },
            depthWrite: {
              default: false,
              description:
            ...
            depthTest: {
              default: true,
              description:
                "if true, don't render a particle's pixels if something is closer in the depth buffer",
            },
            affectedByFog: {
              default: true,
              description:
            ...
            emitterScale: {
              default: 100,
              description:
                'global scaling factor for all particles from the emitter',
            },
            ...
            relative: {
              ...
              oneOf: ['local', 'world'],
              description:
                'world relative particles move relative to the world, while local particles move relative to the emitter (i.e. if the emitter moves, all particles move with it)',
            ...
            duration: {
              default: -1,
              description:
                'duration of the emitter (seconds), if less than 0 then continuously emit particles',
            },
            distribution: {
              default: 'BOX',
              oneOf: ['BOX', 'SPHERE', 'DISC'],
              description:
                "distribution for particle positions, velocities and acceleration. will be overriden by specific '...Distribution' attributes",
            ...
            activeMultiplier: {
              ...
              min: 0,
              description:
                'multiply the rate of particles emission, if larger than 1 then the particles will be emitted in bursts. note, very large numbers will emit all particles at once',
            },
            direction: {
              default: 'forward',
              oneOf: ['forward', 'backward'],
              description:
                'make the emitter operate forward or backward in time',
            ...
            positionDistribution: {
              ...
              description:
                'distribution of particle positions, disc and sphere will use the radius attributes. For box particles emit at 0,0,0, for sphere they emit on the surface of the sphere and for disc on the edge of a 2D disc on the XY plane',
              parse: x => (x || 'NONE').toUpperCase(),
            },
            positionSpread: {
              type: 'vec3',
              description:
                'particles are positioned within +- of these local bounds. for sphere and disc distributions only the x axis is used',
            },
            positionOffset: {
              type: 'vec3',
              description:
                'fixed offset to the apply to the emitter relative to its parent entity',
            },
            randomizePosition: {
              default: false,
              description:
                'if true, re-randomize position when re-spawning a particle, can incur a performance hit',
            },
            radius: {
              default: 1,
              min: 0,
              description:
                'radius of the disc or sphere emitter (ignored for box). note radius of 0 will prevent velocity and acceleration if they use a sphere or disc distribution',
            },
            radiusScale: {
              type: 'vec3',
              default: {x: 1, y: 1, z: 1},
              description:
                'scales the emitter for sphere and disc shapes to form oblongs and ellipses',
            },
            velocityDistribution: {
              default: 'NONE',
              oneOf: ['NONE', 'BOX', 'SPHERE', 'DISC'],
              description:
                "distribution of particle velocities, for disc and sphere, only the x component will be used. if set to NONE use the 'distribution' attribute for velocityDistribution",
              parse: x => (x || 'NONE').toUpperCase(),
            },
            velocity: {
              type: 'vec3',
              description:
                'for sphere and disc distributions, only the x axis is used',
            },
            velocitySpread: {
              type: 'vec3',
            ...
            randomizeVelocity: {
              default: false,
              description:
                'if true, re-randomize velocity when re-spawning a particle, can incur a performance hit',
            },
            accelerationDistribution: {
              default: 'NONE',
              oneOf: ['NONE', 'BOX', 'SPHERE', 'DISC'],
              description:
                "distribution of particle acceleration, for disc and sphere, only the x component will be used. if set to NONE use the 'distribution' attribute for accelerationDistribution",
              parse: x => (x || 'NONE').toUpperCase(),
            },
            acceleration: {
              type: 'vec3',
            ...
            accelerationSpread: {
              type: 'vec3',
              description:
                "spread of the particle's acceleration. for sphere and disc distributions, only the x axis is used",
            },
            randomizeAcceleration: {
              default: false,
              description:
                'if true, re-randomize acceleration when re-spawning a particle, can incur a performance hit',
            },
            drag: {
              default: 0,
              min: 0,
              max: 1,
              description:
                "apply resistance to moving the particle, 0 is no resistance, 1 is full resistance, particle will stop moving at half of it's maxAge",
            },
            dragSpread: {
              default: 0,
            ...
            randomizeDrag: {
              default: false,
              description:
                'if true, re-randomize drag when re-spawning a particle, can incur a performance hit',
            },
            wiggle: {
              default: 0,
              description:
                'extra distance the particle moves over its lifetime',
            },
            wiggleSpread: {
              default: 0,
            ...
            rotationStatic: {
              default: false,
              description:
                "if true, the particles are fixed at their initial rotation value. if false, the particle will rotate from 0 to the 'rotation' value",
            },
            // rotationPivot: {
            ...
            randomizeRotation: {
              default: false,
              description:
                'if true, re-randomize rotation when re-spawning a particle, can incur a performance hit',
            },
            color: {
              type: 'array',
              default: ['#fff'],
              description:
                "array of colors over the particle's lifetime, max 4 elements",
            },
            colorSpread: {
              type: 'array',
              default: [],
              description:
                'spread to apply to colors, spread an array of vec3 (r g b) with 0 for no spread. note the spread will be re-applied through-out the lifetime of the particle',
              parse: x =>
                typeof x === 'string'
                  ? x.split(',').map(AFRAME.utils.coordinates.parse)
                  : x,
              stringify: x =>
                x.map(AFRAME.utils.coordinates.stringify).join(','),
            },
            randomizeColor: {
              default: false,
              description:
                'if true, re-randomize colour when re-spawning a particle, can incur a performance hit',
            },
            opacity: {
              type: 'array',
              default: [1],
              description:
                "opacity over the particle's lifetime, max 4 elements",
            ...
            opacitySpread: {
              ...
              default: [0],
              description:
                "spread in opacity over the particle's lifetime, max 4 elements",
              parse: parseFloatArray,
            },
            randomizeOpacity: {
              default: false,
              description:
                'if true, re-randomize opacity when re-spawning a particle, can incur a performance hit',
            },
            size: {
              type: 'array',
              default: [1],
              description:
                "array of sizes over the particle's lifetime, max 4 elements",
              parse: parseFloatArray,
            },
            sizeSpread: {
              type: 'array',
              default: [0],
              description:
                "spread in size over the particle's lifetime, max 4 elements",
              parse: parseFloatArray,
            },
            randomizeSize: {
              default: false,
              description:
                'if true, re-randomize size when re-spawning a particle, can incur a performance hit',
            },
            angle: {
              type: 'array',
              default: [0],
              description:
                "2D rotation of the particle over the particle's lifetime, max 4 elements",
              parse: parseFloatArray,
            },
            angleSpread: {
              type: 'array',
              default: [0],
              description:
                "spread in angle over the particle's lifetime, max 4 elements",
              parse: parseFloatArray,
            },
            randomizeAngle: {
              default: false,
              description:
                'if true, re-randomize angle when re-spawning a particle, can incur a performance hit',
            },
          },
          ...
          init: function () {
            ...
            this.defaultTexture = new THREE.DataTexture(
          ...
          update: function (oldData) {
            ...
            if (
              Object.keys(delta).some(
                x => !['enabled', 'enableInEditor'].includes(x),
              )
            ) {
          ...
          tickParticleSystem: function (dt) {
            if (this.data.relative === 'world') {
              let newPosition = toVector3(
                this.data.positionOffset,
          ...
          addParticleSystem: function () {
            ...
            let particleTexture = data.texture
              ? textureLoader.load(data.texture)
              : this.defaultTexture;

            let blending = data.blending || 'No';
            blending =
              blending.charAt(0).toUpperCase() +
              blending.substring(1).toLowerCase() +
              'Blending';
            if (!blending in THREE)
              console.error(`unknown blending mode "${data.blending}"`);
            ...
            let groupOptions = {
              texture: {
                ...
                frameCount:
                  data.textureFrameCount >= 0
                    ? data.textureFrameCount
                    : undefined,
            ...
            let emitterOptions = {
              type: SPE.distributions[
                data.distribution in SPE.distributions
                  ? data.distribution
                  : 'BOX'
              ],
              ...
              position: {
                value:
                  data.relative === 'World'
                    ? toVector3(this.data.positionOffset).applyMatrix4(
                        this.el.object3D.matrixWorld,
                      )
                    : toVector3(data.positionOffset).applyMatrix4(
                        this.el.object3D.matrix,
                      ),
                radius: data.radius,
                ...
                distribution:
                  SPE.distributions[
                    data.positionDistribution in SPE.distributions
              ...
              velocity: {
                ...
                distribution:
                  SPE.distributions[
                    data.velocityDistribution in SPE.distributions
              ...
              acceleration: {
                ...
                distribution:
                  SPE.distributions[
                    data.accelerationDistribution in SPE.distributions
                      ? data.accelerationDistribution
                      : data.distribution
                  ], // default to the base distribution
              ...
              color: {
                value:
                  data.color.length > 0
                    ? data.color.map(x => toColor(x))
                    : [toColor('#fff')],
                spread:
                  data.colorSpread.length > 0
                    ? data.colorSpread.map(x => toVector3(x))
                    : undefined, // doesn't accept an empty array
            ...
            this.referenceEl =
              data.relative === 'world' ? this.el.sceneEl : this.el;
            this.referenceEl.setObject3D(
              this.getEmitterName(),
        ...
        function diff(a, b) {
          ...
          for (let k of keys) {
            ...
            let isObj =
              av &&
              bv &&
              ((av.constructor == Object && bv.constructor === Object) ||
                (Array.isArray(av) && Array.isArray(bv)));

            if (
              (isObj && !AFRAME.utils.deepEqual(av, bv)) ||
              (!isObj && av !== bv)
            ) {
              delta[k] = bv;
            }
    ...
    2: [
      function (require, module, exports) {
         ...
         * @return {SPE.TypedArrayHelper} Instance of this class.
         */
        SPE.TypedArrayHelper.prototype.setVec2Components = function (
          index,
          x,
          y,
        ) {
         ...
         * @return {SPE.TypedArrayHelper} Instance of this class.
         */
        SPE.TypedArrayHelper.prototype.setNumber = function (
          index,
          numericValue,
        ) {
          'use strict';

          this.array[this.indexOffset + index * this.componentSize] =
            numericValue;
          return this;
        };
          ...
          randomDirectionVector3OnSphere: (function () {
            ...
            return function (
              ...
              v.normalize().multiplyScalar(
                -this.randomFloat(speed, speedSpread),
              );
         ...
         * @return {Group} This group instance.
         */
        SPE.Group.prototype.triggerPoolEmitter = function (
          numEmitters,
          position,
        ) {
          'use strict';
        ...
        SPE.Emitter = function (options) {
          ...
          options.radius = utils.ensureTypedArg(
            options.radius,
            types.OBJECT,
            {},
          );
          ...
          options.wiggle = utils.ensureTypedArg(
            options.wiggle,
            types.OBJECT,
            {},
          );
          options.maxAge = utils.ensureTypedArg(
            options.maxAge,
            types.OBJECT,
            {},
          );
          ...
          this.wiggle = {
            ...
            _spread: utils.ensureTypedArg(
              options.wiggle.spread,
              types.NUMBER,
              0,
            ),
          };
        ...
        SPE.Emitter.prototype._createGetterSetters = function (
          propObj,
          propName,
        ) {
          'use strict';
        ...
        SPE.Emitter.prototype._activateParticles = function (
          ...
          for (
            var i = activationStart, index, dtValue;
            i < activationEnd;
            ++i
          ) {
            index = i * 4;
